...
with open(fastaFileName, "r") as fastaFile:
   with open(outFileName, "w") as outFile:
      for seqRec in SeqIO.parse(fastaFile, "fasta", Alphabet()):
         ...
         transSeqs = (s[i:(len(s) - ((len(s) - i) % 3))].translate(to_stop=True)
                      for i in range(3) for s in seqs)

         # select the longest one
         transSeq = max(transSeqs, key=len)

         transSeqRec = SeqRecord(
             transSeq, id=seqRec.id, name=seqRec.name, description=seqRec.description)
